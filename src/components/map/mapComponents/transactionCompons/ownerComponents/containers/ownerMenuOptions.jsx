...
import AssignIcon from '@material-ui/icons/AssignmentTurnedIn';
...
class OwnerOptions extends React.Component {
	...
	render() {
		const { BackToTasksWindow, handleUpdateMenu, handleGetAllMenu, handleAssignMenu, classes } = this.props;
			...
			<React.Fragment>
				...
				<Grid container className={classes.index}>
					...
					<Grid item xs={12}>
						<List component="nav">
							...
							<ListItem button onClick={handleUpdateMenu}>
								...
								<ListItemText
									inset
									primary="Create owner"
									secondary="create a new contact by filling in..."
								/>
							</ListItem>
							<Divider variant="middle" />
							<ListItem button onClick={handleAssignMenu}>
								<ListItemIcon>
									<AssignIcon />
								</ListItemIcon>
								<ListItemText
									inset
									primary="Assign owner"
									secondary="Assign owner to this building"
								/>
							</ListItem>
							<Divider variant="middle" />
					...
					<Grid item xs={5} className={classes.buttonSpacing}>
						<Button fullWidth={true} variant={'contained'} onClick={BackToTasksWindow}>
							Back to List
						</Button>
					</Grid>
				</Grid>
...
OwnerOptions.propTypes = {
	classes: PropTypes.object.isRequired
};

export default withStyles(styles)(OwnerOptions);
